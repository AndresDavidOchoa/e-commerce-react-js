...
export const CartProvider = ({ children }) => {
    ...
    const addItem = (product, quantity) => {
        if (!isInCart(product.id)) {
            ...
            //actualizar quantity
            //buscamos el item en el carrito, en el array, su ubicacion para despues poder actualizar el item
            const itemIndex = cart.findIndex((item) => item.id === parseInt(product.id));
            //creamos un borrador del item para poder modificarlo y evitar la modificacion del estado de react
            const itemDraft = { ...cart[itemIndex] };
            //actualizamos la quiantity en el borrador
            itemDraft.quantity = itemDraft.quantity + quantity;
            //creamos un borrador del carrito para poder actualizar el item
            const cartDraft = [...cart];
            // atualizamos el carrito borrador, sin tocar el estado de react
